...
from spike import PrimeHub, LightMatrix, Button, Motor, DistanceSensor, MotorPair
...
# En esta parte indicaras que algoritmos usaras, en este complejo estamos usando el FACE RECOGNITION
huskyLens.algorithm("ALGORITHM_OBJECT_TRACKING")

# Esto sirve para personalizar el nombre del objeto que aprendio tu camara.
huskyLens.setCustomName("Victima Viva", 1)
...
def VictimSearch(Numero):
    while True:
        ...
        if len(vivo) == 1:
            ...
            motor_pair.move_tank(left_speed=100, right_speed=-100)
...
def FollowVictim():
    while True:
        ...
        if len(vivo) == 1:
            ...
            if x_position < 140:  # Si el objeto está a la izquierda del eje X 
                # motor_pair.move(1, "cm", steering=100)
                motor_pair.move_tank(1, "cm", left_speed=10, right_speed=15)
                print("Derecha")
            elif x_position > 180:  # Si el objeto está a la derecha del eje X
                # motor_pair.move(1, "cm", steering=-100)
                print("Izquierda")
                motor_pair.move_tank(1, "cm", left_speed=15, right_speed=10)
            else:
                motor_pair.start(0, 10)

        else: 
            display.show_image("GHOST")
            motor_pair.start_tank_at_power(-35, 40)

def ModelLookingFor():
    while True:
        model0 = huskyLens.loadModelFromSDCard(0)
        model1 = huskyLens.loadModelFromSDCard(1)
        # model2 = huskyLens.loadModelFromSDCard(2)
        # model3 = huskyLens.loadModelFromSDCard(3)
        ...
        if len (ball) == 1:
            ...
            model1
            ball1 = huskyLens.getBlocksByID(1)
            wait_for_seconds(5)
            if len (ball1) == 1:                
                display.show_image("ANGRY")

                x_position = ball1[0].x
            ...
            #     model2
            #     wait_for_seconds(10)
            #     ball2 = huskyLens.getBlocksByID(1)
            #     if len(ball2) == 1:
            #         display.show_image("UMBRELLA")

            #         x_position = ball2[0].x
            ...
            #         model3
            #         wait_for_seconds(10)
            #         ball3 = huskyLens.getBlocksByID(1)
            #         if len(ball3) == 1:
            #             display.show_image("HAPPY")

            #             x_position = ball3[0].x
...
def FollowVictim2():
    while True:
        pelota = huskyLens.getBlocksByID(1)
        ...
        if len(pelota) == 1:
            ...
            x_position = pelota[0].x
            h_height = pelota[0].height
            ...
            display.show_image("GHOST")
            motor_pair.start_tank_at_power(-45, 50)
